...
interface SwalOptions {
	title?: string;
	html: string;
	customClass?: { confirmButton: string; cancelButton: string; denyButton: string };
...
const swalAlertFire = async (selectOptions: SwalOptions): Promise<SweetAlertResult> => {
	const currentOptions = extend(optionsAlert, selectOptions);

	return await Swal.fire(currentOptions);
};

export { swalAlertConfirm, swalAlertInfo, swalAlertConfirmOrDeny, swalAlertFire, type SwalOptions };
